...
function cost_calculator_contact_box_shortcode($atts)
{
	...
	$output .= '<fieldset class="vc_col-sm-6 wpb_column vc_column_container">
			<div class="cost-calculator-block">
				' . (!empty($name_label) && $labels_style=="default" ? '<label>' . $name_label . '</label>' : '') . '
				<input name="name" type="text" value=""' . ((int)$name_required ? ' data-required="1"' : '') . (!empty($name_label) && $labels_style=="placeholder" ? ' placeholder="' . $name_label . '"' : '') . '>
			</div>
			<div class="cost-calculator-block">
				' . (!empty($email_label) && $labels_style=="default" ? '<label>' . $email_label . '</label>' : '') . '
				<input name="email" type="text" value=""' . ((int)$email_required ? ' data-required="1"' : '') . (!empty($email_label) && $labels_style=="placeholder" ? ' placeholder="' . $email_label . '"' : '') . '>
			</div>
			<div class="cost-calculator-block">
				' . (!empty($phone_label) && $labels_style=="default" ? '<label>' . $phone_label . '</label>' : '') . '
				<input name="phone" type="text" value=""' . ((int)$phone_required ? ' data-required="1"' : '') . (!empty($phone_label) && $labels_style=="placeholder" ? ' placeholder="' . $phone_label . '"' : '') . '>
			</div>
		</fieldset>
		<fieldset class="vc_col-sm-6 wpb_column vc_column_container">
			<div class="cost-calculator-block cost-calculator-textarea-block">
				' . (!empty($message_label) && $labels_style=="default" ? '<label>' . $message_label . '</label>' : '') . '
				<textarea name="message"' . ((int)$message_required ? ' data-required="1"' : '') . (!empty($message_label) && $labels_style=="placeholder" ? ' placeholder="' . $message_label . '"' : '') . '></textarea>
			</div>
		</fieldset>';
	if(!empty($label))
		$output .= '</div>';
	$output .= '</div>
	<div class="vc_row wpb_row vc_inner cost-calculator-contact-box-submit-container' . ((int)$cost_calculator_global_form_options["google_recaptcha"] && empty($description) ? ' cost-calculator-fieldset-with-recaptcha' : ((int)$cost_calculator_global_form_options["google_recaptcha"] && !empty($description) ? ' cost-calculator-row-with-recaptcha' : '')) . '">';
	if(!empty($description))
	{
		$output .= '<div class="vc_col-sm-6 wpb_column vc_column_container">
			<p>' . $description . '</p>
		</div>
		<div class="vc_col-sm-6 wpb_column vc_column_container' . ((int)$cost_calculator_global_form_options["google_recaptcha"] ? ' cost-calculator-column-with-recaptcha' : '') . '">';
	}
	...
	if((int)$terms_checkbox)
	{
		...
		if((int)$cost_calculator_global_form_options["google_recaptcha"])
		{
			$output .= '<div class="cost-calculator-recaptcha-container">';
		}
	}
	$output .= '<div class="vc_row wpb_row vc_inner' . ((int)$cost_calculator_global_form_options["google_recaptcha"] ? ' cost-calculator-button-with-recaptcha' : '') . '">
			<a class="cost-calculator-more cost-calculator-submit-form" href="#" title="' . esc_attr($submit_label) . '"><span>' . $submit_label . '</span></a>
		</div>';
	if((int)$cost_calculator_global_form_options["google_recaptcha"])
	{
		if($cost_calculator_global_form_options["recaptcha_site_key"]!="" && $cost_calculator_global_form_options["recaptcha_secret_key"]!="")
		{
			...
			$output .= '<p>' . __("Error while loading reCapcha. Please set the reCaptcha keys under Theme Options in admin area", 'cost-calculator') . '</p>';
		if((int)$terms_checkbox)
		{
			$output .= '</div>';
		}
	...
	vc_map( array(
		...
		"params" => array(
			...
			array(
				...
				"value" => array(__("Default", 'cost-calculator') => "default", __("Placeholder", 'cost-calculator') => "placeholder")
			),
			array(
				"type" => "dropdown",
				"class" => "",
			...
			array(
				...
				"value" => ((int)$cost_calculator_global_form_options["google_recaptcha"] ? __("Yes", 'cost-calculator') : __("No", 'cost-calculator')),
...
function cost_calculator_form()
{
	...
	{
		if(array_key_exists($key . "_required_field", $_POST) && ($value=="" || $value==null || ((int)$value==0 && array_key_exists($key . "_required_field_is_checkbox", $_POST))))
		{
	...
	if($requiredCheck && ((isset($_POST["terms"]) && (int)$_POST["terms"]) || !isset($_POST["terms"])) && (((int)$cost_calculator_global_form_options["google_recaptcha"] && !empty($_POST["g-recaptcha-response"])) || !(int)$cost_calculator_global_form_options["google_recaptcha"]) && ((isset($_POST["name_required"]) && (int)$_POST["name_required"] && $_POST["name"]!="") || (!isset($_POST["name_required"]) || !(int)$_POST["name_required"])) && ((isset($_POST["email_required"]) && (int)$_POST["email_required"] && $_POST["email"]!="" && preg_match("#^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-zA-Z]{2,12})$#", $_POST["email"])) || (!isset($_POST["email_required"]) || !(int)$_POST["email_required"])) && ((isset($_POST["phone_required"]) && (int)$_POST["phone_required"] && $_POST["phone"]!="") || (!isset($_POST["phone_required"]) || !(int)$_POST["phone_required"])) && ((isset($_POST["message_required"]) && (int)$_POST["message_required"] && $_POST["message"]!="") || (!isset($_POST["message_required"]) || !(int)$_POST["message_required"])))
	{
		...
		if(((int)$cost_calculator_global_form_options["google_recaptcha"] && isset($verify_recaptcha["success"]) && (int)$verify_recaptcha["success"]) || !(int)$cost_calculator_global_form_options["google_recaptcha"])
		{
			if((int)$cost_calculator_global_form_options["send_email"] || (int)$cost_calculator_global_form_options["send_email_client"] || (int)$cost_calculator_global_form_options["save_calculation"])
			{
				...
				$subject = $cost_calculator_contact_form_options["email_subject"];
				...
				if((int)$cost_calculator_global_form_options["send_email"])
				{
					...
					$headers[] = 'From: ' . $cost_calculator_contact_form_options["admin_name"] . ' <' . $cost_calculator_contact_form_options["admin_email"] . '>' . "\r\n";
					...
					{
						$result["submit_message"] = $cost_calculator_contact_form_options["thankyou_message"];
					}
					else
					{
						$result["isOk"] = false;
						$result["error_message"] = $GLOBALS['phpmailer']->ErrorInfo;
						$result["submit_message"] = $cost_calculator_contact_form_options["error_message"];
					}
				...
				if((int)$cost_calculator_global_form_options["send_email_client"] && $values["email"]!="")
				{
					...
					$headers_client[] = 'From: ' . $cost_calculator_contact_form_options["admin_name"] . ' <' . $cost_calculator_contact_form_options["admin_email"] . '>' . "\r\n";
			...
			{
				$result["isOk"] = false;
				$result["submit_message"] = $cost_calculator_contact_form_options["error_message"] . " " . __("Sending disabled by administrator", 'cost-calculator');
			}
		}
		else
		{
	...
	else
	{
		...
		if(isset($_POST["name_required"]) && (int)$_POST["name_required"] && $_POST["name"]=="")
			$result["error_name"] = $cost_calculator_contact_form_options["name_message"];
		if(isset($_POST["email_required"]) && (int)$_POST["email_required"] && ($_POST["email"]=="" || !preg_match("#^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-zA-Z]{2,12})$#", $_POST["email"])))
			$result["error_email"] = $cost_calculator_contact_form_options["email_message"];
		if(isset($_POST["phone_required"]) && (int)$_POST["phone_required"] && $_POST["phone"]=="")
			$result["error_phone"] = $cost_calculator_contact_form_options["phone_message"];
		if(isset($_POST["message_required"]) && (int)$_POST["message_required"] && $_POST["message"]=="")
			$result["error_message"] = $cost_calculator_contact_form_options["message_message"];
		if((int)$cost_calculator_global_form_options["google_recaptcha"] && empty($_POST["g-recaptcha-response"]))
			$result["error_captcha"] = $cost_calculator_contact_form_options["recaptcha_message"];
		if(isset($_POST["terms"]) && !(int)$_POST["terms"])
			$result["error_terms"] = $cost_calculator_contact_form_options["terms_message"];
	}
