...
function cost_calculator_dropdown_box_shortcode($atts)
{
	...
	$output = '<div class="cost-calculator-box cost-calculator-clearfix' . ($top_margin!="none" ? ' ' . esc_attr($top_margin) : '') . '">';
	if($label!="")
	{
	...
	for($i=0; $i<$options_count; $i++)
		$output .= '<option value="' . (isset($atts["option_value" . $i]) ? esc_attr($atts["option_value" . $i]) : "") . '"' . (!empty($default_value) && $atts["option_value" . $i]==$default_value ? ' selected="selected"' : '') . '>' . (!empty($atts["option_name" . $i]) ? $atts["option_name" . $i] : '') . '</option>';
	$output .= '</select>';
	...
	$output .= '<input type="hidden" class="' . esc_attr($id) . '" name="' . esc_attr($name) . '-name" value="' . esc_attr($default_name) . '">
	</div>';
	return $output;
}
