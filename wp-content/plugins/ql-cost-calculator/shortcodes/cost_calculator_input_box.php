...
function cost_calculator_input_box_shortcode($atts)
{
	...
	if($type!="hidden" && $type!="submit")
		$output .= '<div class="cost-calculator-box cost-calculator-clearfix' . ($type=="checkbox" && $checkbox_type=="type-button" ? ' cost-calculator-float' : '') . ($top_margin!="none" ? ' ' . esc_attr($top_margin) : '') . ($el_class!="" && $type!="checkbox" ? ' ' . esc_attr($el_class) : '') . '">';
	if($label!="" && ($type!="checkbox" || ($type=="checkbox" && $checkbox_type=="default")) && $type!="hidden" && $type!="submit")
		$output .= '<label for="' . esc_attr($id) . '"' . ((int)$hide_label ? ' class="cost-calculator-hidden"' : '') . '>' . $label . '</label>';
	if($label!="" && $type!="hidden" && $type!="submit")
	...
	vc_map( array(
		...
		"params" => array(
			...
			array(
				...
				"param_name" => "type",
				"value" => array(__("text", 'cost-calculator') => "text", __("number", 'cost-calculator') => "number", __("date", 'cost-calculator') => "date", __("email", 'cost-calculator') => "email", __("checkbox", 'cost-calculator') => "checkbox", __("hidden", 'cost-calculator') => "hidden", __("submit", 'cost-calculator') => "submit")
			),
			array(
				"type" => "textfield",
				"class" => "",
