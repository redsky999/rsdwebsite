...
function costCalculatorLoadShortcode() 
{
			...
			success: function(json){
				...
				if(json!==0)
				{
					...
					jQuery.each(json.advanced_settings, function(key, val){
						if(key!="primary_font_subset" && key!="secondary_font_subset")
						{
							jQuery("#" + key).val(val).keyup();
							if(key=="calculator_skin")
								jQuery("#calculator_skin").selectmenu("refresh").trigger("selectmenuchange");
							if(key=="primary_font")
								jQuery("#primary_font").selectmenu("refresh").trigger("selectmenuchange", [json.advanced_settings.primary_font_subset]);
							if(key=="secondary_font")
								jQuery("#secondary_font").selectmenu("refresh").trigger("selectmenuchange", [json.advanced_settings.secondary_font_subset]);
	...
	else
	{
		...
		if(jQuery("#cost-calculator-form").length)
		{
			...
			jQuery(".cost-calculator-skin-dropdown, .cost-calculator-fonts-dropdown").selectmenu("refresh").trigger("selectmenuchange");
		}
		jQuery("#shortcode-delete").css("display", "none");
...
function costCalculatorChangeSkin(event)
{
	...
	{
		jQuery("#main_color, #box_color, #text_color, #border_color, #label_color, #form_label_color, #inactive_color, #primary_font_custom, #primary_font, #secondary_font_custom, #secondary_font").val("").keyup().prev().css("background-color", "#" + jQuery(this).data("color"));
		if(event.type=="selectmenuchange")
			jQuery("#primary_font, #secondary_font").selectmenu("refresh").trigger("selectmenuchange");
		else
			jQuery("#primary_font, #secondary_font").trigger("change");
	}
	else if(jQuery(this).val()=="carservice")
	{
		...
		jQuery("#form_label_color").val("").keyup().prev().css("background-color", "#" + jQuery(this).data("color"));
		jQuery("#inactive_color").val("E2E6E7").keyup().prev().css("background-color", "#" + jQuery("#inactive_color").val());
		jQuery("#primary_font").val("Open Sans:regular");
		jQuery("#primary_font_custom, #secondary_font_custom, #secondary_font").val("");
		if(event.type=="selectmenuchange")
		{
			jQuery("#primary_font").selectmenu("refresh").trigger("selectmenuchange", [['latin', 'latin-ext']]);
			jQuery("#secondary_font").selectmenu("refresh").trigger("selectmenuchange");
		}
		else
		{
			jQuery("#primary_font").trigger("change", [['latin', 'latin-ext']]);
			jQuery("#secondary_font").trigger("change");
		}
	}
	else if(jQuery(this).val()=="renovate")
	{
		...
		jQuery("#form_label_color").val("").keyup().prev().css("background-color", "#" + jQuery(this).data("color"));
		jQuery("#inactive_color").val("E2E6E7").keyup().prev().css("background-color", "#" + jQuery("#inactive_color").val());
		jQuery("#primary_font_custom, #primary_font, #secondary_font_custom").val("");
		jQuery("#secondary_font").val("Raleway:300");
		if(event.type=="selectmenuchange")
		{
			jQuery("#primary_font").selectmenu("refresh").trigger("selectmenuchange");
			jQuery("#secondary_font").selectmenu("refresh").trigger("selectmenuchange", [['latin', 'latin-ext']]);
		}
		else
		{
			jQuery("#primary_font").trigger("change");
			jQuery("#secondary_font").trigger("change", [['latin', 'latin-ext']]);
		}
	}
}
function costCalculatorLoadFontSubsets(event, params)
{
	var self = jQuery(this);
	if(self.val()!="")
	{
		...
		jQuery.ajax({
			url: ajaxurl,
			type: 'post',
			data: "action=cc_get_font_subsets&font=" + jQuery(this).val(),
			success: function(data){
				data = jQuery.trim(data);
				var indexStart = data.indexOf("cc_start")+8;
				var indexEnd = data.indexOf("cc_end")-indexStart;
				data = data.substr(indexStart, indexEnd);
				var subsetsContainer = self.parent().parent().next();
				var subsetsSelect = subsetsContainer.find("select.font-subset");
				subsetsContainer.removeClass("cost-calculator-hidden");
				subsetsSelect.html(data);
				if(jQuery.type(params)=="array")
					subsetsSelect.val(params);
				spinner.css({
					"display": "none",
					"visibility": "hidden",
				});
...
jQuery(document).ready(function($){
		...
		buttons: [
			{
				...
				click: function(){
					...
					($("#" + form_id + ' [name="currency_position"]').length ? ' currency_position="' + $("#" + form_id + ' [name="currency_position"]').val() + '"' : "") +
					($("#" + form_id + ' [name="thousandth_separator"]').length ? ' thousandth_separator="' + $("#" + form_id + ' [name="thousandth_separator"]').val() + '"' : "") +
	...
	$("#cost-calculator-form").on("submit", function(event) {
		...
		builderContent.find(".cost-calculator-column").each(function(){
			...
			$(this).after('[vc_column width="' + columnWidth + '"]' + $(this).html() + '[/vc_column]');
			$(this).detach();
		});
	...
	$(".cost-calculator-fonts-dropdown").each(function(){
		$(this).selectmenu({
			icons: {button: "cc-plugin-arrow-select"},
			change: costCalculatorLoadFontSubsets
		});
	});
	$(".cost-calculator-fonts-dropdown").on("selectmenuchange", costCalculatorLoadFontSubsets);
	$("#primary_font:not('.cost-calculator-fonts-dropdown'), #secondary_font:not('.cost-calculator-fonts-dropdown')").on("change", costCalculatorLoadFontSubsets);
	//recaptcha
	...
	{
		$("#row-modal, .cost-calculator-element-modal").dialog("option", "position", {my: "center", at: "center", of: window});
	}
