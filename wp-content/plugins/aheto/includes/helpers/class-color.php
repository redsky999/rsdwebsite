...
class Color {
	 ...
	 */
	public static function newColor( $color, $mode = 'auto' ) { // @codingStandardsIgnoreLine
	 ...
	 * @param  int    $lightness Amount to lighten.
	 * @return string
	 */
	public static function lighten( $color, $lightness ) {
		$color     = self::newColor( $color );
		$lightness = $color->lightness + $lightness;
		$lightness = 100 < $lightness ? 100 : $lightness;
		return $color->getNew( 'lightness', $lightness )->toCSS( 'hex' );
	}
	 ...
	 * @param  int    $darkness Amount to darken.
	 * @return string
	 */
	public static function darken( $color, $darkness ) {
		$color    = self::newColor( $color );
		$darkness = $color->lightness - $darkness;
		$darkness = 0 > $darkness ? 0 : $darkness;
		return $color->getNew( 'lightness', $darkness )->toCSS( 'hex' );
	}

	/**
	 * Set alpha to the given color
	 *
	 * @param  string $color   Color to darken.
	 * @param  int    $alpha Amount to fade out.
	 * @return string
	 */
	public static function setAlpha( $color, $alpha ) { // @codingStandardsIgnoreLine
		$color = self::newColor( $color );
		return $color->getNew( 'alpha', $alpha )->toCSS( 'rgba' );
	}

	/**
	 * Fade out the given color
	 *
	 * @param  string $color   Color to darken.
	 * @param  int    $opacity Amount to fade out.
	 * @return string
	 */
	public static function fadeOut( $color, $opacity ) { // @codingStandardsIgnoreLine
		$color   = self::newColor( $color );
		$opacity = max( 0, min( 1, 1 - $opacity ) );
		return $color->getNew( 'alpha', $opacity )->toCSS( 'rgba' );
	}
	 ...
	 */
	public function getNew( $property = '', $value = '' ) { // @codingStandardsIgnoreLine

		if ( in_array( $property, [ 'red', 'green', 'blue', 'alpha' ], true ) ) {
			// Check if we're changing any of the rgba values.
			$value = max( 0, min( 255, $value ) );
			if ( 'red' === $property ) {
				return self::newColor( 'rgba(' . $value . ',' . $this->green . ',' . $this->blue . ',' . $this->alpha . ')', 'rgba' );
			} elseif ( 'green' === $property ) {
				return self::newColor( 'rgba(' . $this->red . ',' . $value . ',' . $this->blue . ',' . $this->alpha . ')', 'rgba' );
			} elseif ( 'blue' === $property ) {
				return self::newColor( 'rgba(' . $this->red . ',' . $this->green . ',' . $value . ',' . $this->alpha . ')', 'rgba' );
			} elseif ( 'alpha' === $property ) {
				return self::newColor( 'rgba(' . $this->red . ',' . $this->green . ',' . $this->blue . ',' . $value . ')', 'rgba' );
			}
		} elseif ( in_array( $property, [ 'hue', 'saturation', 'lightness' ], true ) ) {
			// Check if we're changing any of the hsl values.
			$value = ( 'hue' === $property ) ? max( 0, min( 360, $value ) ) : max( 0, min( 100, $value ) );

			if ( 'hue' === $property ) {
				return self::newColor( 'hsla(' . $value . ',' . $this->saturation . '%,' . $this->lightness . '%,' . $this->alpha . ')', 'hsla' );
			} elseif ( 'saturation' === $property ) {
				return self::newColor( 'hsla(' . $this->hue . ',' . $value . '%,' . $this->lightness . '%,' . $this->alpha . ')', 'hsla' );
			} elseif ( 'lightness' === $property ) {
				return self::newColor( 'hsla(' . $this->hue . ',' . $this->saturation . '%,' . $value . '%,' . $this->alpha . ')', 'hsla' );
			}
			...
			return self::newColor( 'rgba(' . $red . ',' . $green . ',' . $blue . ',' . $this->alpha . ')', 'rgba' );
		}
	 ...
	 * @return string
	 */
	public function toCSS( $mode = 'hex' ) { // @codingStandardsIgnoreLine
	...
	protected function set_luminance() {
		$luminance       = ( 0.2126 * $this->red ) + ( 0.7152 * $this->green ) + ( 0.0722 * $this->blue );
