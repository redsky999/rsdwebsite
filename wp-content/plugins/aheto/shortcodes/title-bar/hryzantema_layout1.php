...
if ( (empty( $custom_css ) || ( $custom_css == "disabled" ) ) && !Helper::is_Elementor_Live()) {
	wp_enqueue_style('hryzantema-title-bar-layout1', $shortcode_dir . 'assets/css/hryzantema_layout1.css', null, null);
}
?>
				...
				<?php if ( !empty($searchform) ) : ?>
					<div class="aheto-titlebar__input <?php echo Helper::get_button($this, $atts, 'sf_', true); ?>">
						<form role="search" class="w-800" method="get" id="searchform"
							...
							<label class="screen-reader-text" for="s"><?php esc_html_e('Search:', 'hryzantema') ?></label>
							<input type="text" value="" name="s" id="s"
								   placeholder="<?php echo esc_attr($sf_placeholder); ?>"/>
							<?php if ( !empty( $sf_button ) ) { ?>
								<input type="submit" id="searchsubmit" value="<?php echo esc_attr($sf_button); ?>"/>
							<?php } else { ?>
