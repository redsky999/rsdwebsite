...
$this -> add_render_attribute ( 'wrapper', 'class', 'aheto-contents--bizy-faq' );
...
if ((empty($custom_css) || ($custom_css == "disabled")) && !Helper::is_Elementor_Live()) {
	wp_enqueue_style ( 'bizy-contents-layout2', $shortcode_dir . 'assets/css/bizy_layout2.css', null, null );
}
...
<div <?php $this -> render_attribute_string ( 'wrapper' ); ?>>
	...
	foreach ($faqs as $key => $item) :
		...
		?>
        <div class="aheto-contents__item bizy-js-accordion-item <?php echo esc_attr ( $class_active ); ?>">
			<?php if ( !empty( $item['title'] )) : ?>
                <h5 class="aheto-contents__title js-accordion"><?php echo wp_kses ( $item['title'], 'post' ); ?><span
                            class="action-icon"></span></h5>
			<?php endif; ?>
