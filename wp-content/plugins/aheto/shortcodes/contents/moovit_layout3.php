...
$this->add_render_attribute( 'wrapper', 'class', 'aheto-contents--moovit-faq' );
...
$icon_style = isset($moovit_icon_size) && !empty($moovit_icon_size) ? 'style="font-size: '. $moovit_icon_size .'"' : '';
...
if ( (empty( $custom_css ) || ( $custom_css == "disabled" ) ) && !Helper::is_Elementor_Live()) {
	wp_enqueue_style( 'moovit-contents-layout3', $shortcode_dir . 'assets/css/moovit_layout3.css', null, null );
} ?>
        ...
        <div class="aheto-contents__item <?php echo esc_attr( $class_active ); ?>">
			<?php if ( ! empty( $item['title'] ) ) : ?>
                <h5 class="aheto-contents__title js-accordion"><?php echo wp_kses( $item['title'], 'post' ); ?>
                    <i <?php echo $icon_style; ?>  class="ion-ios-arrow-down"></i></h5>
			<?php endif; ?>
