...
$enable_light = isset( $snapster_cf_enable_light ) && ! empty( $snapster_cf_enable_light ) ? 'enable_light' : '';

$this->generate_css();
$this->add_render_attribute( 'wrapper', 'id', $element_id );
$this->add_render_attribute( 'wrapper', 'class', $this->the_custom_classes() );
$this->add_render_attribute( 'wrapper', 'class', $enable_light );
$this->add_render_attribute( 'wrapper', 'class', 'aheto-pricing-tables--snapster' );
$this->add_render_attribute( 'wrapper', 'class', 'aheto-pricing-tables--snapster-cf_modern' );

$random             = substr( md5( rand() ), 0, 7 );
$placeholder        = isset( $placeholder ) && ! empty( $placeholder ) ? $placeholder : esc_html__( 'List 3 things about you I should know', 'snapster' );
...
if ( (empty( $custom_css ) || ( $custom_css == "disabled" ) ) && !Helper::is_Elementor_Live()) {
	wp_enqueue_style( 'snapster-pricing-tables-layout8', $shortcode_dir . 'assets/css/snapster_layout8.css', null, null );
}
if (!Helper::is_Elementor_Live()) {
	wp_enqueue_script('snapster-pricing-tables-layout8-js', $shortcode_dir . 'assets/js/snapster_layout8.min.js', array('jquery'), null);
...
<div <?php $this->render_attribute_string( 'wrapper' ); ?>>
	...
	<?php if ( ! empty( $snapster_cf_mail ) ) { ?>
        <div class="aheto-pricing-tables__form <?php echo Helper::get_button( $this, $atts, 'snapster_cf_modern_', true ); ?>">
            <form id="aheto-pricing-tables__pricelistform" class="aheto-pricing-tables__send-email"
                ...
                <input type="text" name="snapster_date"
                       placeholder="<?php esc_html_e( 'Date', 'snapster' ); ?>">

                <input type="text" name="snapster_location"
                       placeholder="<?php esc_html_e( 'Location', 'snapster' ); ?>">
