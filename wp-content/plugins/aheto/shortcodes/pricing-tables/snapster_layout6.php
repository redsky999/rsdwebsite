...
$pricing_items = $this->parse_group( $snapster_with_media );
...
$this->add_render_attribute( 'wrapper', 'class', 'aheto-pricing-tables--snapster-with_media' );
...
if ( (empty( $custom_css ) || ( $custom_css == "disabled" ) ) && !Helper::is_Elementor_Live()) {
	wp_enqueue_style( 'snapster-pricing-tables-layout6', $shortcode_dir . 'assets/css/snapster_layout6.css', null, null );
}
if (!Helper::is_Elementor_Live()) {
	wp_enqueue_script('snapster-pricing-tables-layout6-js', $shortcode_dir . 'assets/js/snapster_layout6.min.js', array('jquery'), null);
}
...
<div <?php $this->render_attribute_string( 'wrapper' ); ?>>
	...
	<?php foreach ( $pricing_items as $item ) {
		...
		$background_image = ! empty( $item['snapster_bg_image'] ) ? \Aheto\Helper::get_background_attachment( $item['snapster_bg_image'], $snapster_with_media_image_size, $atts, 'snapster_with_media_' ) : '';
		$video_class      = ! empty( $background_image ) ? 'enable-video' : ''; ?>

		<?php if ( ! empty( $background_image ) ) { ?>
            <div class="aheto-pricing-tables__media-wrap <?php echo esc_attr( $video_class ); ?>" <?php echo esc_attr( $background_image ); ?>>
				<?php if ( $item['snapster_with_media_add_video_button'] && ! empty( $item['snapster_with_media_video_link'] ) ) {
					echo \Aheto\Helper::get_video_button( $item, 'snapster_with_media_' );
				} ?>
            </div>
		<?php } ?>

        <div class="aheto-pricing-tables__wrap">
            <div class="aheto-pricing-tables__text-wrap">
					...
					if ( ! empty( $item['snapster_price'] ) ) { ?>
                        ...
                        <div class="aheto-pricing-tables__price-wrap">
                            ...
                            <div class="aheto-pricing-tables__input-check-wrap">
                                <input type="radio"
                                       name="<?php echo esc_attr( $random ); ?>with_media"
					...
					if ( ! empty( $item['snapster_list'] ) ) {
                        ...
                        <ul class="aheto-pricing-tables__list-wrap">
							<?php foreach ( $list as $listitem ) {
								$icon_font  = $item['snapster_with_media_icon_font'];
								$icon_style = '';
								if ( ! empty( $item['snapster_with_media_icon_fz'] ) || ! empty( $item['snapster_with_media_icon_color'] ) ) {
									$icon_style = 'style=';
								}
								$icon_style .= ! empty( $item['snapster_with_media_icon_fz'] ) ? 'font-size:' . $item['snapster_with_media_icon_fz'] . ';' : '';
								$icon_style .= ! empty( $item['snapster_with_media_icon_color'] ) ? 'color:' . $item['snapster_with_media_icon_color'] . ';' : ''; ?>
                                <li>
									<?php if ( $item['snapster_with_media_add_icon'] && ! empty( $icon_font ) ) { ?>
                                        <i class="<?php echo esc_attr( $item[ 'snapster_with_media_icon_' . $icon_font ] ); ?>" <?php echo esc_attr( $icon_style ); ?>></i>
									<?php }
