...
$this->add_render_attribute( 'wrapper', 'class', 'portfolio-nav--acacio-wrap' );
 ...
 * Set dependent style
 */
$shortcode_dir = aheto()->plugin_url() . 'shortcodes/portfolio-nav/';
$custom_css    = Helper::get_settings( 'general.custom_css_including' );
$custom_css    = ( isset( $custom_css ) && ! empty( $custom_css ) ) ? $custom_css : false;
if ( (empty( $custom_css ) || ( $custom_css == "disabled" ) ) && !Helper::is_Elementor_Live()) {
    wp_enqueue_style('acacio-portfolio-nav-layout1', $shortcode_dir . 'assets/css/acacio_layout1.css', null, null);
    ...
    <div class="portfolio-nav <?php echo esc_attr( $prev_post_class . $next_post_class )?>">
        ...
        if ( !empty( $prev_post ) ) {
            ...
            $prev = get_previous_post_link(
                ...
                '<div class="portfolio-nav__dir-image"><img src="' . $prev_post_image . '" alt="' . $prev_post_alt . '" class="js-bg"></div>
					  <h6 class="portfolio-nav__dir-title">' . $prev_post->post_title . '<p>' . esc_html__( 'Prev', 'acacio' ) . '</p></h6>',
                false,
                '',
        ...
        if ( !empty($next_post) ) {
            ...
            $prev = get_next_post_link(
                ...
                '<h6 class="portfolio-nav__dir-title">' . $next_post->post_title . '<p>' . esc_html__( 'Next', 'acacio' ) . '</p></h6>
